(ns iata_timatic.main
  "Kotodama WASM entrypoint for the Iata_timatic clean-room actor (L4) — Clojure port.
  ...
  facts (`iata_timatic.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "iata_timatic")
(def tier "L4")
...
(def entity-specs
  [{:entity "TradeItem" :plural "tradeitems" :id-prefix "iatatima_tra"
    :fields [:gtin :description :brand]          :required [:gtin :description]
    :coerce {}                       :refs {}}
   {:entity "Event"     :plural "events"     :id-prefix "iatatima_eve"
    :fields [:itemId :bizStep :disposition :recordedAt] :required [:bizStep :disposition]
    :coerce {}                       :refs {}}
   {:entity "Document"  :plural "documents"  :id-prefix "iatatima_doc"
    :fields [:type :senderId :receiverId :status]     :required [:type :status]
    :coerce {}                       :refs {}}
   {:entity "Container" :plural "containers" :id-prefix "iatatima_con"
    :fields [:bicCode :type :status]              :required [:bicCode :type]
    :coerce {}                       :refs {}}
   {:entity "Party"     :plural "parties"     :id-prefix "iatatima_par"
    :fields [:gln :name :role]                   :required [:gln :name]
    :coerce {}                       :refs {}}
   {:entity "Shipment"  :plural "shipments"  :id-prefix "iatatima_shi"
...
(defn emit-facts
  "EAVT facts for one record: {\"iata_timatic.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "iata_timatic-compat" :tier tier :entities entities} 200])
...
(defn start! [] :iata_timatic-compat/ready)
